...
def getWeatherDataByCoords(lat, lng):
    r = requests.get('https://api.openweathermap.org/data/2.5/weather?lat={}&lon={}&appid=36e1555b9828764965dd60b0976e0eda'.format(lat, lng))
    loc_weather = r.content.strip()
    jsonData = json.loads(loc_weather)
    clouds = jsonData['clouds']['all']
    icon = jsonData['weather'][0]['icon']
